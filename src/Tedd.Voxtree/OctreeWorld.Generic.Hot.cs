...
public sealed partial class OctreeWorld<T> where T : unmanaged
{
    ...
    /// </remarks>
    public bool TryMarkChunkHot(int chunkX, int chunkY, int chunkZ, out HotOctreeChunk<T>? hotChunk)
    {
        OctreeChunk<T>? source;
        bool knownEmpty;
        using (ReadLock())
        {
        ...
        hotChunk = knownEmpty
            ? HotOctreeChunk<T>.FromKnownEmpty(ChunkLevels, ChannelCount)
            : source!.MarkHot();
        return true;
    }

    /// <summary>Creates a mutable Morton-order editor, throwing when the chunk is unloaded.</summary>
    public HotOctreeChunk<T> MarkChunkHot(int chunkX, int chunkY, int chunkZ)
    {
    ...
    /// </remarks>
    public bool TryCommitHotChunk(int chunkX, int chunkY, int chunkZ, HotOctreeChunk<T> hotChunk)
    {
    ...
    public OctreeChunk<T> CommitHotChunk(int chunkX, int chunkY, int chunkZ, HotOctreeChunk<T> hotChunk)
    {
    ...
    public OctreeChunk<T> UnmarkChunkHot(int chunkX, int chunkY, int chunkZ, HotOctreeChunk<T> hotChunk) =>
