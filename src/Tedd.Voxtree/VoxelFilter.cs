...
public readonly struct VoxelFilter
{
    ...
    {
        if (_mask == 0) return _negate ? 0 : values.Length;
        ...
        if (values.Length >= 8) return MaskedVoxelSearch<uint>.Count(values, _mask, _expected, _negate);
#endif
        var count = 0;
        foreach (var value in values)
            count += Matches(value) ? 1 : 0;
        return count;
    }
    ...
    {
        if (_mask == 0) return !values.IsEmpty && !_negate;
#if NET10_0_OR_GREATER
        if (_mask == uint.MaxValue)
            return _negate ? values.IndexOfAnyExcept(_expected) >= 0 : values.IndexOf(_expected) >= 0;
        if (values.Length >= 8) return MaskedVoxelSearch<uint>.Any(values, _mask, _expected, _negate);
#endif
        foreach (var value in values)
